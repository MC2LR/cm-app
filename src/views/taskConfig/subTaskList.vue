    ...
    <div class="sub-wraper">
      <el-form :inline="true" :model="formInline">
        <el-form-item label="任务名称">
          ...
          <el-input v-model="formInline.subtaskName" placeholder="子任务名称"></el-input>
        </el-form-item>
      ...
      >
        ...
        <el-table-column prop="tsiPerson" label="责任人"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    unitDatas() {
      ...
      httpService.request(api.getSubAll, finData, "post").then(res => {
        const data = httpService.parseResp(res);
        if (data.success) {
          this.tableDatas = data.data;//将数据赋给表格进行渲染
        }
      });
    },
    //删除事件
    handleDeless(datas) {
      const paramse = {
        content: datas.row.tsiContent,
        id: datas.row.id,
        name: datas.row.tsiName,
        person: datas.row.tsiPerson,
        tiName:datas.row.tiName
      };
      this.$confirm("是否删除?", "提示", {
        ...
        .then(() => {
          //点击确定后发送新增请求
          httpService.request(api.subTaskDelete, paramse, "post").then(res => {
            const data = httpService.parseResp(res);
            console.log(data);
            if (data.success) {
              this.$message({
              ...
              this.tableDatas.splice(datas.$index, 1);//删除成功后前端手动删除表格
            } else {
              this.$message({
                type: "info",
                message: "删除失败"
    ...
    //编辑按钮事件
    handleClicksss(datas) {
      this.$router.push({
        name: "subTaskAdd",
        params: { setId: datas.row.id } //将需要修改行的id和组件类型传到新增数据页
      });
    },
    //查询按钮
    onSubmit() {
      const finData = {
        tiName: this.formInline.taskName,
        person: this.formInline.subdutyPerson,
        name:this.formInline.subtaskName
      };
      httpService.request(api.getSubAll, finData, "post").then(res => {
        const data = httpService.parseResp(res);
